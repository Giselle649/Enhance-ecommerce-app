...
const CartsPage = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const {data} = await axios.get("https://3.25.165.74" + "/cart", {headers: {
    ...
    const fetchAddress = async () => {
        try {
            ...
            //clg dulu data, dan liat di console browser, bukan liat di postman
              ...
              const response = await axios.get("https://3.25.165.74" + "/user", {headers: {
                Authorization: `Bearer ${token}`,
            }})
...
const deleteButtonHandler = async (e, productId) => {
    try {
        ...
        const token = localStorage.getItem('access_token');
        //clg dulu data, dan liat di console browser, bukan liat di postman
        await axios.delete("https://3.25.165.74" + `cart/${productId}`, {headers: {
            Authorization: `Bearer ${token}`,
          }})
          fetchData();
...
const formSubmitHandler = async (e, productId) => {
    try {
        ...
        //clg dulu data, dan liat di console browser, bukan liat di postman
        await axios.post("https://3.25.165.74" + "/transaction", data, {headers: {
            Authorization: `Bearer ${token}`,
          }})

        let response = await axios.post("https://3.25.165.74" + "/generate-midtrans-token", {realTotal: realTotal}, {headers: {
          Authorization: `Bearer ${token}`,
        }})
        window.location.href =response.data.redirect_url

        
        await axios.delete("https://3.25.165.74" + "/cart", {headers: {
          Authorization: `Bearer ${token}`,
        }})
